...
export const GET: APIRoute = async ({ params, locals }) => {
  const tripId = params.tripId; const expenseId = params.expenseId;
  if (!tripId || !expenseId) return new Response(JSON.stringify({ error: 'Missing identifiers' }), { status: 400 });
  const supabase = locals.supabase; if (!supabase) return new Response(JSON.stringify({ error: 'Supabase client not available' }), { status: 500 });
  ...
  const result: any = { ...expense, category: expense.budget_categories, budget_categories: undefined, trips: undefined };
...
export const PUT: APIRoute = async ({ params, locals, request }) => {
  ...
  if (!body || typeof body !== 'object') return new Response(JSON.stringify({ error: 'Invalid JSON body' }), { status: 400 });
  const parsed = updateExpenseSchema.safeParse(body);
  if (!parsed.success) return new Response(JSON.stringify({ error: 'Validation failed', details: parsed.error.issues }), { status: 422 });
  if (Object.keys(parsed.data).length === 0) return new Response(JSON.stringify({ error: 'No fields to update' }), { status: 400 });
