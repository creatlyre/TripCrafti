...
export const POST: APIRoute = async ({ request, params, locals }) => {
	...
	let categoriesPayload: Array<z.infer<typeof createCategorySchema>> = [];

	if (Array.isArray(body)) {
		// Accept raw array for backward simplicity
		const arrParse = z.array(createCategorySchema).safeParse(body);
		if (!arrParse.success) {
			return new Response(JSON.stringify({ error: 'Validation failed', details: arrParse.error.issues }), { status: 422 });
		}
		mode = 'bulk';
		...
		if (!bulkParse.success) {
			return new Response(JSON.stringify({ error: 'Validation failed', details: bulkParse.error.issues }), { status: 422 });
		}
		mode = 'bulk';
		...
		if (!singleParse.success) {
			return new Response(JSON.stringify({ error: 'Validation failed', details: singleParse.error.issues }), { status: 422 });
		}
