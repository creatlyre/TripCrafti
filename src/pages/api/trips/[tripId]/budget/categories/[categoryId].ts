...
export const PUT: APIRoute = async ({ params, locals, request }) => {
  const tripId = params.tripId; const categoryId = params.categoryId;
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  if ((count ?? 0) > 0) return new Response(JSON.stringify({ error: 'Category in use', code: 'CATEGORY_IN_USE', references: count }), { status: 409 });
