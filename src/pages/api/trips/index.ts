...
export const GET: APIRoute = async ({ locals }) => {
  ...
  let { data, error } = await supabase
    .from('trips')
    .select(
      `*, itineraries:generateditineraries (id, trip_id, preferences_json, generated_plan_json, status, input_tokens, thought_tokens, created_at, updated_at)`
    )
    .eq('user_id', user.id)
  ...
  if (error && /input_tokens|thought_tokens|column/.test(error.message)) {
    ...
    const retry = await supabase
      .from('trips')
      .select(
        `*, itineraries:generateditineraries (id, trip_id, preferences_json, generated_plan_json, status, created_at, updated_at)`
      )
      .eq('user_id', user.id)
      .order('start_date', { ascending: true });
    data = retry.data;
