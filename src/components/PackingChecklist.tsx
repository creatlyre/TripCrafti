  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 mt-4 no-print">
      <input
        ...
        placeholder={listT?.addTaskPlaceholder || 'Dodaj nowe zadanie...'}
...
const EditableChecklistItem: React.FC<EditableChecklistItemProps> = ({
  item,
  onToggleItem,
  onUpdateItem,
  onDeleteItem,
    ...
    return (
      <li className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 p-2 rounded-md no-print">
        ...
        <button
          ...
          aria-label={ui?.save || 'Zapisz zmianę'}
        >
          <svg
        ...
        <button
          ...
          aria-label={ui?.cancel || 'Anuluj edycję'}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5"
    ...
    <li className="group flex items-center bg-slate-50 dark:bg-slate-700/50 p-3 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
      ...
      <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity no-print">
        <button
          ...
          aria-label={ui?.edit || 'Edytuj zadanie'}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-4 w-4"
        ...
        <button
          ...
          aria-label={ui?.remove || 'Usuń zadanie'}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-4 w-4"
      ...
      <div className="mb-2 flex flex-col gap-2">
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">
            {useDictionary().packing?.checklist?.heading || 'Przed Wyjazdem (Checklista)'}
          </h2>
          <button
            type="button"
          ...
          >
            {collapsed ? (
              <>
                <span>▼</span>
                <span>{useDictionary().ui?.common.expand || 'Rozwiń'}</span>
              </>
            ) : (
              <>
                <span>▲</span>
                <span>{useDictionary().ui?.common.collapse || 'Zwiń'}</span>
              </>
            )}
          </button>
        </div>
        {!collapsed && totalCount > 0 && (
          <div className="mt-1 no-print">
            <div className="flex justify-between items-center text-sm text-slate-500 dark:text-slate-400">
              <span>{useDictionary().ui?.common.progress || 'Postęp'}</span>
              <span>
