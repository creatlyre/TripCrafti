...
export function usePacking({ tripId, autoSave = true, autoSaveDelay = AUTO_SAVE_DELAY }: UsePackingOptions): UsePackingReturn {
  // Core state
  ...
  const packingStats = useMemo(() => 
    PackingService.getPackingStats(packingItems), 
    [packingItems]
  );
  ...
  useDebounce(
    saveData,
    autoSaveDelay,
    [packingItems, checklistItems, categories, listMeta]
  );
  ...
  const validateList = useCallback(async (context?: string) => {
  ...
  // Item management
  const addItem = useCallback((name: string, category: string, quantity: string, notes?: string) => {
  ...
  const addItemFromLibrary = useCallback((item: ItemDefinition) => {
  ...
  const sortCategories = useCallback((sortBy: 'az' | 'za' | 'count') => {
  ...
  const updateChecklistItem = useCallback((itemId: number, task: string) => {
    setChecklistItems(prev => prev.map(item => 
      item.id === itemId ? { ...item, task: task.trim() } : item
    ));
  }, []);
  ...
  // Suggestions
  const applySuggestions = useCallback((suggestions: ValidationResult) => {
