...
function getRegionFromDestination(destination: string): string {
    ...
    if (/\b(warszawa|kraków|gdańsk|wrocław|polska|poland|warsaw|krakow|gdansk|wroclaw)\b/.test(dest)) return 'Europa Środkowa';
    if (/\b(sztokholm|oslo|helsinki|kopenhaga|skandynawia|szwecja|norwegia|finlandia|dania|sweden|norway|finland|denmark|stockholm|helsinki|copenhagen)\b/.test(dest)) return 'Europa Północna';
    ...
    if (/\b(nowy jork|los angeles|san francisco|chicago|usa|america|new york|francisco)\b/.test(dest)) return 'Ameryka Północna';
