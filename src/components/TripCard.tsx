      ...
      >
        <div onClick={() => onOpen()} className="cursor-pointer">
          ...
          <CardContent className="text-sm text-slate-400 space-y-2 flex-grow">
            ...
            <div className="mt-4">
               <a href={`/app/${trip.id}`} onClick={e=>e.stopPropagation()} className="inline-block text-xs uppercase tracking-wide text-indigo-400 hover:text-indigo-300 font-semibold px-3 py-1 rounded-full border border-indigo-500/30 bg-indigo-500/10 hover:bg-indigo-500/20 transition-colors">
                 {dict.open} {"→"}
               </a>
            </div>
          </CardContent>
        </div>
        <CardFooter className="flex flex-wrap items-center justify-end gap-2 p-4 mt-auto">
           {dict.deleteAction && (
             <button
               type="button"
               onClick={() => onDelete && onDelete(trip)}
               className="text-sm text-red-400 hover:text-red-300 underline decoration-dotted mr-auto"
             >
               {dict.deleteAction}
             </button>
           )}
           <a
             ...
             className="text-xs px-3 py-1.5 rounded-md border border-slate-700 text-slate-300 hover:bg-indigo-600 hover:text-white hover:border-indigo-500 transition-colors"
