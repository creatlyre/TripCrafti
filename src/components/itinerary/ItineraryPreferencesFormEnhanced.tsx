...
const preferencesSchema = z
  .object({
    ...
    kidsCount: baseNumber('Invalid')
      ...
      .or(z.nan().transform(() => undefined)),
    kidsAges: z
      .array(baseNumber('Invalid').int('Invalid').min(0, 'Age >= 0').max(17, '<18'))
      .optional()
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <span
                        className={`text-xs font-medium ${isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}
                      >
                        {interestObj.label}
                      </span>
                    </div>
            ...
            <div className="space-y-2">
              {travelStyleOptions.map((style) => (
                ...
                >
                  ...
                  >
                    {watch('travelStyle') === style.value && <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />}
                  </div>
                  <div className="flex-1">
                    <span
                      className={`font-medium text-sm ${watch('travelStyle') === style.value ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}
                    >
                      {style.label}
                    </span>
                    {style.description && (
            ...
            {tripBudget ? (
              ...
              <div className="space-y-2">
                {budgetOptions.map((budget) => (
                  ...
                  >
                    <input type="radio" {...register('budget')} value={budget.value} className="sr-only" />
                    <div
                      className={`w-3 h-3 rounded-full border-2 mr-3 flex items-center justify-center transition-colors ${
                        watch('budget') === budget.value ? 'border-blue-500' : 'border-slate-300 dark:border-slate-600'
                      }`}
                    >
                      {watch('budget') === budget.value && <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />}
                    </div>
                    <span
                      className={`font-medium text-sm ${watch('budget') === budget.value ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}
                    >
                      {budget.label}
                    </span>
                  </label>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-slate-100">
              ...
              {dict?.tooltip?.travelParty && (
                <span title={dict.tooltip.travelParty} className="cursor-help text-slate-400 dark:text-slate-500">
                  ⓘ
                </span>
              )}
            </label>
              ...
              <div className="space-y-1">
                ...
                {errors.adultsCount && (
                  <p className="text-xs text-red-600 dark:text-red-400">
                    {validationMap[errors.adultsCount.message || ''] || errors.adultsCount.message}
                  </p>
                )}
              </div>
              <div className="space-y-1">
                ...
                {errors.kidsCount && (
                  <p className="text-xs text-red-600 dark:text-red-400">
                    {validationMap[errors.kidsCount.message || ''] || errors.kidsCount.message}
                  </p>
                )}
              </div>
            </div>
            ...
            {(() => {
              ...
              <div className="space-y-2">
                  ...
                  {Array.from({ length: Math.min(20, watch('kidsCount') || 0) }).map((_, idx) => (
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setValue('kidsAges', next.filter((v) => v !== undefined) as any, {
                          shouldDirty: true,
                          shouldValidate: true,
                        });
                      }}
                ...
                {errors.kidsAges && (
                  <p className="text-xs text-red-600 dark:text-red-400">
                    {validationMap[errors.kidsAges.message || ''] || (errors.kidsAges.message as any)}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-slate-100">
              ...
              {dict?.tooltip?.lodging && (
                <span title={dict.tooltip.lodging} className="cursor-help text-slate-400 dark:text-slate-500">
                  ⓘ
                </span>
              )}
            </label>
            <div className="space-y-2">
              <Input
                ...
                className="text-sm"
              />
              {errors.hotelNameOrUrl && (
                <p className="text-xs text-red-600 dark:text-red-400">{errors.hotelNameOrUrl.message}</p>
              )}
              <div className="flex items-center gap-2">
                ...
                {errors.maxTravelDistanceKm && (
                  <p className="text-xs text-red-600 dark:text-red-400">
                    {validationMap[errors.maxTravelDistanceKm.message || ''] || errors.maxTravelDistanceKm.message}
                  </p>
                )}
                {dict?.tooltip?.distance && (
                  <span title={dict.tooltip.distance} className="cursor-help text-slate-400 dark:text-slate-500">
                    ⓘ
                  </span>
                )}
              </div>
