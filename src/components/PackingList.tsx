    ...
    if (isLoading && totalItemCount === 0) {
        return (
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md text-center packing-list-container">
                <p className="text-slate-500 dark:text-slate-400">Generowanie listy, proszę czekać...</p>
            </div>
        );
    }
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        <div className="relative flex-grow no-print">
                         ...
                         <button
                            ...
                            aria-label={isCategorizedView ? "Pokaż listę płaską" : "Pokaż kategorie"}
                          >
                            {isCategorizedView ? (
                ...
                {isCategorizedView && (
                    <div className="flex items-center gap-2 flex-wrap mb-4 no-print">
                        <button
                            ...
                            aria-label={areAllExpanded ? "Zwiń wszystko" : "Rozwiń wszystko"}
                            disabled={categories.length === 0}
                        >
                            {areAllExpanded ? "Zwiń wszystko" : "Rozwiń wszystko"}
                        </button>
                        ...
                        <button onClick={() => onSortCategories('count')} className="px-2 py-1 text-xs font-semibold text-slate-600 bg-slate-100 rounded-md hover:bg-slate-200 transition-colors dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600" aria-label="Sortuj po liczbie przedmiotów"># Przedmiotów</button>
                    </div>
                )}
            </div>
                                            ...
                                            <div className="flex items-center flex-grow text-left group">
                                                ...
                                                <div className="ml-3 flex-grow">
                                                    {editingCategoryName === category ? (
                                                        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                                            <input
                                                                ...
                                                                autoFocus
                                                            />
                                                            <button onClick={handleEditCategorySave} className="p-1 text-green-600 hover:text-green-800" aria-label="Zapisz nazwę">✓</button>
                                                            <button onClick={handleEditCategoryCancel} className="p-1 text-red-600 hover:text-red-800" aria-label="Anuluj">×</button>
                                                        </div>
                                                    ) : (
                                                 ...
                                                 <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity ml-2 no-print" onClick={(e) => e.stopPropagation()}>
                                                     <button onClick={() => handleEditCategoryStart(category)} className="p-1 text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400" aria-label="Edytuj kategorię">
                                                         <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L16.732 3.732z" /></svg>
                                                     </button>
                                                     <button onClick={() => onDeleteCategory(category)} disabled={categoryItems.length > 0} className="p-1 text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-500 disabled:text-slate-300 dark:disabled:text-slate-600 disabled:cursor-not-allowed" aria-label="Usuń kategorię">
                                        ...
                                        >
                                            <div className={`pt-1 pb-2 pl-2 pr-1 ${isExpanded ? 'mt-2' : ''}`}>
                                                {categoryItems.length > 0 ? (
                                                    ...
                                                    <p className="text-sm text-slate-400 italic px-2">Brak przedmiotów w tej kategorii.</p>
                                                )}
